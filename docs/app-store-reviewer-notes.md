...
- **Organization:** Shuttler Demo (org code: `demo-appstore`)
...
Sign in with the email option, select "Shuttler Demo" in the org selector. The demo org is pre-seeded with shuttle routes, stops, announcements, and boarding history.
...
- Org owners: Org Setup → Billing tab → Danger Zone → **Delete Organization** (immediately cancels every remaining Stripe subscription — including add-ons — and permanently deletes all organization data and memberships; requires a two-step destructive confirmation). Members' own login accounts remain and can be removed via Delete Account.
